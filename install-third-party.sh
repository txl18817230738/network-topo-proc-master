...
ARGS=`getopt -a -o V:PhH -l version:,prefix:,help -- "$@"`
function usage() {
    echo  '
Usage:
 install-third-party.sh <optstring> <parameters>
 install-third-party.sh [options] [--] <optstring> <parameters>

Example:
 install-third-party.sh --prefix=/opt/vesoft/third-party/5.0 -V 5.0

Parse command options.

Options:
 -V, --version                 specific the install version of third-party
 -P, --prefix                  specific the install prefix of third-party
...
bash $selected_archive --prefix=$prefix $@ && rm -rf $selected_archive
