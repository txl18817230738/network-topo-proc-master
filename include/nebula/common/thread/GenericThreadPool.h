...
#include "nebula/common/thread/GenericWorker.h"
...
class GenericThreadPool final : public nebula::NonCopyable, public nebula::NonMovable {
public:
    GenericThreadPool();
    ~GenericThreadPool();
...
template <typename F, typename... Args>
uint64_t GenericThreadPool::addTimerTask(size_t delay,
                                         size_t interval,
                                         F &&f,
                                         Args &&... args) {
    auto idx = nextThread_++ % nrThreads_;
    auto id = pool_[idx]->addTimerTask(
            delay, interval, std::forward<F>(f), std::forward<Args>(args)...);
    return ((idx << GenericWorker::TIMER_ID_BITS) | id);
}
